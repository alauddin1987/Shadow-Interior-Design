...
public static function generateMenuArray($id, &$output, &$child, $maxlevel=10, $level=0,$previous_level=0,$level_separator='===' )
{
		$level_str = '';
		for($i=0;$i<$level;$i++)
...
function buildChild($symtemMenu,$parent_id='main_catid')
{
		$child = array();
...
{
	$status = ($totalRec >= 1)?TOTAL_RECORD.'&nbsp;'.self::convertToBanglaNumber($totalRec).NOS."&nbsp;".VIEW.self::convertToBanglaNumber($ViewStart).'&nbsp;'.FROM.'&nbsp;'.self::convertToBanglaNumber($ViewEnd).'&nbsp;'.TO:NO_RECORD_FOUND;
	return $status;
}
